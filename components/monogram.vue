...
export default {
  ...
  render() {
    const popup = ()=>( <div
      class="absolute z-40 w-40 text-sm font-medium cursor-auto animate-slide-left popup text-center justify-around flex flex-col top-0 left-full ml-1 shadow-lg bg-water/10 backdrop-blur-md border-[0.5px] border-water/20 text-white rounded-lg px-3 py-2">
      ...
      <div class="text-base font-bold">{[this.user.firstName, this.user.lastName].join(' ')}</div>
    ...
    return (
      <div class="inline-flex relative gap-2 " style="">
        {this.icon && <div
          vOn:click={() => (!this.huge && this.$store.commit('elimination/setPopup', {id: this.id, type: 0}))}
          class={`rounded-full popup-trigger ${!this.huge && 'cursor-pointer hover:shadow-md'} justify-center text-center ${!this.huge&&'relative'}  shrink-0 inline-flex ${colors[this.n]} text-white font-semibold  ${this.small ? 'w-6 h-6 grow text-xs' : (this.huge ? 'w-16 h-16 text-2xl tracking-wide' : 'w-7 h-7 text-xs')}`}>
          <span class="my-auto">{(this.user.firstName || [])[0]}{(this.user.lastName || [])[0]}</span>
          <span
        ...
        { !(this.icon || this.name) && <div vOn:click={() => (this.$store.commit('elimination/setPopup', {id:this.id, type:2}))} class="popup-trigger bg-white/20 font-semibold text-xs px-0.5 rounded-md cursor-pointer hover:bg-white/30">@{this.user.firstName} {this.user.lastName}
